...
//#include "your model file(library from edge pulses) .h"
...
void ei_use_result(ei_impulse_result_t result) {
  ...
  ESP_LOGI(TAG, "FOUNDDDD :  %d",count);
  if(top_label && low_label){
    ESP_LOGI(TAG, "Upper is %s",top_label);
    ESP_LOGI(TAG, "Lower is %s",low_label);
  }
